...
export function CreateSprintModal({ 
  ...
  const onSubmit = async (data: SprintFormValues) => {
    ...
    await addSprint(projectId, {
      name: data.name,
      goal: data.goal || "",
      status: "Planned",
      startDate: data.startDate ? new Date(data.startDate).toISOString() : new Date().toISOString(),
      endDate: data.endDate ? new Date(data.endDate).toISOString() : new Date(Date.now() + 14 * 24 * 60 * 60 * 1000).toISOString(),
    });
      ...
      <div className="relative w-full max-w-md z-10 animate-fade-in-up">
        <Card className="p-6 sm:p-8">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-xl font-semibold tracking-tight text-foreground">Create Sprint</h2>
